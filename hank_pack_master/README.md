...
15. https://mirrors.tools.huawei.com/maven/org/glassfish/json/1.0.4/
    ...
    这是一个远程仓库的文件目录，要求，创建 maven/org/glassfish/json/1.0.4/ 这种层级的文件夹到 .m2（mavenLocal）目录，并下载这链接下的所有文件 （DONE）
16. 关于XGate引起的快速上传流程，我想做成 当网络变化时，自动检查未上传成功的 产物，按照之前选择的上传方式，逐个上传，但是仍然保留手动上传的入口。